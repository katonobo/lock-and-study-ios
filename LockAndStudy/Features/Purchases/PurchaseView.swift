...
struct PurchaseView: View {
  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 22) {
        HStack { Text("学び方を選ぶ").font(.largeTitle.bold()); Spacer(); Button { dismiss() } label: { Image(systemName: "xmark.circle.fill").font(.title2) }.accessibilityLabel("閉じる") }
        purchaseSection(title: "一つの目標に集中", subtitle: "教材を個別に買い切り。サブスクリプション不要で、購入教材を永久利用できます。") {
          ...
          if commerce.entitlement.activePass?.permitsAccess == true, !showPermanentPurchase {
            Label("対象教材はStudy Passに含まれています", systemImage: "checkmark.seal.fill").foregroundStyle(LockAndStudyTheme.brand)
        ...
        purchaseSection(title: "複数の目標・最新版を利用", subtitle: "Study Passは対象教材、新教材、最新年度版、横断学習サービスを利用できます。") {
          ForEach(passProducts) { product in productButton(product, recommended: product.kind == .passYearly) }
        }
        ...
        if let message = commerce.state.message { Text(message).font(.footnote).foregroundStyle(.secondary).accessibilityIdentifier("purchase.state") }
        Text("自動更新はApp Storeのアカウント設定から、更新日の24時間以上前までに解約できます。表示価格・期間はApp Storeの商品情報が正本です。").font(.footnote).foregroundStyle(.secondary)
        HStack { Link("利用規約", destination: AppConfiguration.termsOfUseURL); Link("プライバシーポリシー", destination: AppConfiguration.privacyPolicyURL) }.font(.footnote)
      }.frame(maxWidth: 720).padding()
    }
    .accessibilityIdentifier("purchase.screen")
    .task { if commerce.products.isEmpty { await commerce.loadProducts() } }
  }
  ...
  private func purchaseSection<Content: View>(title: String, subtitle: String, @ViewBuilder content: () -> Content) -> some View {
    VStack(alignment: .leading, spacing: 12) { Text(title).font(.title2.bold()); Text(subtitle).foregroundStyle(.secondary); content() }.studyCard()
  }
  private func productButton(_ product: StoreProductPresentation, recommended: Bool = false) -> some View {
