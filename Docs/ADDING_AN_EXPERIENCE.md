...
- SHA-256、item count、可能な場合はbyte countをカタログへ記録する。
...
## Content schema validatorの登録

`flashcard.items.v1`、`certification.questions.v1`、補助用の`sample.index.v1`は標準Registryに登録済みである。Custom Experienceが独自JSON、SQLite、画像、音声、動画を使う場合は`ContentFileValidating`を実装し、`ContentFileValidatorRegistry`のcomposition rootへ渡す。validatorは自身の`schemaID`だけを担当する。`ContentPackageStore`や`ContentRepository`へschema名のswitchを追加しない。

binary教材は最低byte数と許可拡張子だけなら`OpaqueBinaryContentValidator`を利用できる。独自containerの場合はheader、内部index、codec等を専用validatorで確認する。path、SHA-256、`byteCount`はvalidator呼出し前に共通層が確認する。

資格・ケース問題の`certification.questions.v1`では、独自の正解判定をValidatorやRepositoryへ重複実装しない。`CertificationQuestionWireDecoder`をstage時と実行時の両方で使用する。choice ID、legacy文字列choiceの正規化、`correctChoiceID`／`correctIndex`、誤答rationale、説明の規則を変更する場合はcanonical wire modelとそのテストを先に更新する。stageを通過したwire dataは同じdecoderを使うruntime decodeでも成功しなければならない。

## 同一Packのcontent version更新

Item IDを維持できる説明・誤字修正はmigration文書なしで進捗を保持する。問題内容変更またはID変更がある場合は、package内にmigration JSONを置き、manifestへ`progressMigrationFile`と`progressMigrationSHA256`を記録する。

- 変更なし: `preserve`
- 正解・論点が変わった項目: 同じold/new IDで`resetChangedItems`
- ID変更: old/new IDと`migrate`

`defaultPolicy`は`itemMigrations`に列挙されない既存進捗へ適用する。`preserve`は保持、`resetChangedItems`は削除して初期化、`migrate`は未列挙進捗が1件でもあれば拒否する。個別mappingはdefaultより優先する。

文書にはschema version 1、対象`packID`、正確なfrom/to content versionを必ず含める。別Packの進捗は指定できない。activation失敗テスト、二重実行テスト、rollbackテストを教材packageごとに追加する。
# v11 authoring requirements

Flashcard教材は空状態文言を`presentation.flashcard.emptyStateCopy`へ定義する。`noDueReview`、`noMistakes`、`noWeakItems`、`noNewItems`、`noAvailableItems`を教材の語彙に合わせる。`supportsExamples=false`の場合、コンテンツに例文値が存在しても予習・学習UIは例文を表示しない。

Custom Experienceの復元payload schemaまたはcontent versionを変更すると、進行中Unlock EnvelopeはFail Closedになる。互換性を維持できない更新は新schema IDまたは新Pack版として提供し、既存pack/item/product IDを再利用しない。
