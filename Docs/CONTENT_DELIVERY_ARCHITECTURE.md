...
`ContentPackageValidator`は各descriptorのpath、SHA-256、byte count、schema、item countを検証した後、同じschemaの全ファイルをまとめてpackage-level validatorへ渡す。資格教材は`CertificationQuestionPackagePolicy`をStageとRuntime Repositoryで共有し、複数ファイル横断のID重複、公開合計件数、placeholder、reviewStatus、無料sample IDをactive化前に検証する。したがって、単独ファイルでは正常でも結合後に不正となるpackageはStageでFail Closedとなり、Runtimeまで持ち越さない。
