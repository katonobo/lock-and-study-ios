...
## v13で実装済みのactivation transaction

v13では上記の進捗checkpointと`active.json`の間に永続Activation Journalを追加した。`prepared`、`migrationApplied`、`pointerCommitted`をatomic保存し、起動後の`recoverInterruptedActivations()`が旧pointer／新pointer、package、migration digest、checkpointを照合する。commit前の停止は旧版へ戻し、pointer切替後の停止は完全検証できる場合だけ新版を確定する。rollback成功後の`previousContentVersion`は`nil`であり、前進には明示的な再activateが必要である。

資格問題は`CertificationQuestionWireDecoder`をPackage Validatorと`TakkenQuestionRepository`で共有する。進捗移行の未列挙項目には`defaultPolicy`を適用し、`resetChangedItems`は初期化、`migrate`は全件mapping必須とする。
...
# v11 catalog recovery contract

- Global fatal（重複Category/Series/Pack、Category cycle、root/schema不正）は候補Catalog全体をrejectする。
- Pack-localな不整合は対象Packだけを隔離し、正常Packを継続提供する。
- 採用Catalogはsanitize後のbytes、schema version、generatedAt、validation timestamp、将来用signature metadataとともに`ValidatedCatalogStore`へ保存する。
- 書込みは一時ファイル、同期、atomic replaceの順で行い、直前版をbackupとして保持する。
- cold launchでも永続LKGを利用し、LKG破損時はbundled、最後に組み込みsafe fallbackへ戻る。
