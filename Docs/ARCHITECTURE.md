...
Safe Fallbackは解除回復専用であり、通常教材、Category、オンボーディング、保存済み選択の自動補正には利用しない。利用可能な通常Packが0件の場合、Rootは空のExperienceを開かず`CatalogRecoveryView`を表示する。再読込と設定確認を提供し、ロック中だけ組み込み安全問題へ進める。Shield起点の回復はCatalog全損時もSafe Fallback Runtimeを直接利用できる。

オンボーディングと教材一覧は`normalManifests`を共通の正本とする。初期選択は保存済みの利用可能Pack、次にCatalog順の先頭Packで決まり、タイトル、アイコン、テーマはPack、Category、Experience Descriptorから取得する。「コンテンツと出典」はPackを列挙し、年度、content version、qualification metadata、`creditsFile`を動的に表示する。

Content packageは共通層で相対path、SHA-256、任意の`byteCount`を検証し、内容は`ContentFileValidatorRegistry`へ登録されたschema validatorが検証する。標準RegistryはFlashcard、Certification、補助用Sample Indexを提供する。画像、音声、動画等のCustom schemaは`ContentFileValidating`を登録し、Coreのswitchを変更しない。

Installed packageのactivation前に`ProgressMigrationService`が移行文書のSHA-256、schema、pack、from/to versionを検証する。`LearningDataStore`はPack内の`ItemProgress`だけを`preserve`、`resetChangedItems`、`migrate`で変更し、回答履歴は保持する。移行前snapshotと冪等checkpointを保存し、失敗時は旧active pointerを維持、rollback時は旧進捗を復元する。
