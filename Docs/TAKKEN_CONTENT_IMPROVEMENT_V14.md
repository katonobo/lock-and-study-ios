...
- `reviewedAt`: 実在するISO-8601の日付または日時
- `reviewNote`: 修正・確認内容を具体的に記録
- `sourceNote`: 8文字以上の具体的な根拠（法令名・条文・e-Gov・国土交通省資料など）
...
- `【AI草稿】`、入力欄、AI誤答根拠候補、要校閲などの草稿markerをゼロにする
- `true_false`: 正規化後も「正しい」「誤り」の2択だけにする
- `number_choice`: 数値・比率・期間・金額の短い選択肢だけを使い、単位を省略・混在させない
