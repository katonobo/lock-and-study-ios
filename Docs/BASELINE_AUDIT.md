...
複合pack/item ID、権利と教材状態の中央access decision、固定解除snapshot、原子的ローカル保存、4商品StoreKit catalog、旧アプリの検証済みclaim移行を新規設計しました。その後、Vocabulary/Takken Study Experienceを完全に独立させ、選択教材を単一Rootとして表示する構造へ更新しました。通常学習は教材固有型、解除境界だけは共通Codable snapshotを使います。教材変更は各Experienceの設定内にあるラインナップから行います。
